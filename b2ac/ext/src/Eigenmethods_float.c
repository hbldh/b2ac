...
void QRAlgorithm_float(float *A, float *eigenvalues)
{
	...
	matrixToHessenbergForm_Givens_float(B);
	...
	while (m > 0)
	{
		...
		performQRStep_Givens_float(B, m);
		...
		if (fabsf(B[mMapping[m] - 1]) < QR_ALGORITHM_TOLERANCE_FLOAT)
		{
...
void matrixToHessenbergForm_Givens_float(float *A)
{
	...
	// with columns [1,2].
	givensRotation_float(givensValues, A[3], A[6]);
...
void performQRStep_Givens_float(float *A, int m)
{
	...
	float tau_one = 0.0;
	float tau_two = 0.0;
	float givensValues[4] = {0.0, 0.0, 0.0, 0.0};
	...
	if (m == 2)
	{
		...
		for (k = 0; k < 2; k++)
		{
			...
			givensRotation_float(&givensValues[giv_ind],
									 A[row_ind + k],
									 A[row_ind + 3 + k]);
			for (i = 0; i < 3; i++)
			{
	...
	{
		// Find the Givens rotation to apply here.
		givensRotation_float(givensValues, A[0], A[3]);
		...
		float eigenvalue, int32_t numberOfIterations)
{
	...
	// In the float case, we might need to divide by determinant to
	// avoid overflowing into an INF value...
	determinant = matrixInverse3by3_float(A, inverse);
	if (determinant > 5e10)
	{
		for(i = 0; i < 9; i++)
		{
	...
	for (i = 0; i < numberOfIterations; i++)
	{
		...
		norm_value = sqrtf(tmp_storage[0] * tmp_storage[0] +
